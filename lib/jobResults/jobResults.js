...
class JobResults extends EventEmitter {
    ...
    async _getResultsByFilter(filter) {
        ...
        Object.entries(list).forEach(([k, v]) => {
            const [, , jobId, type] = k.split('/');
            let job = results.find(j => j.jobId === jobId);
            if (!job) {
                job = { jobId };
                results.push(job);
            }
            job[type] = this.parent._tryParseJSON(v);
        });
    ...
    async getResultsLog(options) {
        return this._get({ suffix: 'logResults', jobId: options.jobId });
    }

    async setResultsLog(options) {
        return this._set({ suffix: 'logResults', data: options.data, jobId: options.jobId });
    }

    async getStatusLog(options) {
        return this._get({ suffix: 'logStatus', jobId: options.jobId });
    }

    async setStatusLog(options) {
        return this._set({ suffix: 'logStatus', data: options.data, jobId: options.jobId });
    }
