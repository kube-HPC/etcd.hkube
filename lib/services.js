...
const pipelineDriver = require('./services/services.pipelineDriver');
...
class services {
    ...
    async set(options) {
        ...
        if (setSchemaInstance.valid) {
            const { data, instanceId, serviceName, suffix } = setSchemaInstance.instance;
    ...
    get(options) {
        return new Promise((res, rej) => {
            ...
            if (getSchemaInstance.valid) {
                let { serviceName, instanceId, index, etcdOptions, suffix } = getSchemaInstance.instance;
    ...
    delete(options) {
        return new Promise((res, rej) => {
            const getSchemaInstance = this.getSchema(options, { ignoreNull: true });
            if (getSchemaInstance.valid) {
                let { serviceName, instanceId, etcdOptions, suffix } = getSchemaInstance.instance;
                let internalServiceName = serviceName ? serviceName : this.parent._serviceName
                let _path = path.join('/', PREFIX.SERVICES, internalServiceName, instanceId, suffix);
