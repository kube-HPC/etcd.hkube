...
class Service extends EventEmitter {
    ...
    releaseChangeLock(keyProp) {
        const key = this.getPath(keyProp);
        this._locker.release(watchTypes.CHANGE, key);
    }

    releaseDeleteLock(keyProp) {
        const key = this.getPath(keyProp);
        this._locker.release(watchTypes.DELETE, key);
    }
    ...
    async _handleWatch(event, res, isLock) {
        ...
        if (isLock) {
            const lock = await this._locker.acquire(event, key);
            if (lock) {
