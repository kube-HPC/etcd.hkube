...
class pipelineDriver {
    ...
    async setTaskState(taskId, data) {
        let suffix = path.join('/', SUFFIX.tasks, taskId);
    ...
    async getTaskState(taskId) {
        let suffix = path.join('/', SUFFIX.tasks, taskId);
        return await this.services.get({ instanceId: this.etcd.jobId, suffix })
    ...
    async getDriverTasks() {
        ...
        try {
            ...
            results = result.node.nodes.map(node => {
                ...
                return Object.assign({}, { taskID: taskID }, this._tryParseJSON(node.value));
            });
        }
        catch (error) {
    ...
    async getState() {
        ...
        try {
            ...
            const result = await this.services.get({ instanceId: this.etcd.jobId, suffix });
            results = this._tryParseJSON(result.node.value);
        }
        catch (error) {
            if (error.errorCode === 100) {
                return null;
    ...
    async setState(data) {
        ...
        await this.services.set({ instanceId: this.etcd.jobId, suffix, data });
    }
