...
const PREFIX = require('../consts').PREFIX.PIPELINES;
...
class Pipelines extends EventEmitter {
    ...
    async watch(options) {
        ...
        watch.watcher.on('put', async (res) => {
            ...
            if (options.lock) {
                const lock = await this._locker.acquire('change', [pipeline]);
                if (lock) {
        ...
        watch.watcher.on('delete', async (res) => {
            const [, , pipeline] = res.key.toString().split('/');
            if (options.lock) {
                const lock = await this._locker.acquire('delete', [pipeline]);
                if (lock) {
