...
const LOCK_KEY = `${PREFIX.PIPELINE_DRIVER_QUEUE}/locks`;

class PipelineDriverQueue extends EventEmitter {
    constructor() {
        super();
    ...
    async _delete(options) {
        ...
        const _path = path.join('/', PREFIX.PIPELINE_DRIVER_QUEUE, name);
    ...
    async _set(options) {
        ...
        const _path = path.join('/', PREFIX.PIPELINE_DRIVER_QUEUE, instance.name);
    ...
    async watch(options) {
        ...
        watch.watcher.on('put', async (res) => {
            ...
            if (options.lock) {
                const result = await this._client.lock(`${LOCK_KEY}/change/${alg}`);
                if (result) {
        ...
        watch.watcher.on('delete', async (res) => {
            const [, , alg] = res.key.toString().split('/');
            if (options.lock) {
                const result = await this._client.lock(`${LOCK_KEY}/delete/${alg}`);
                if (result) {
