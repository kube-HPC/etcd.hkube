...
class PipelineDriverQueue extends EventEmitter {
    ...
    async _delete(options) {
        ...
        if (schema.valid) {
            ...
            const _path = path.join('/', PREFIX.PIPELINE_DRIVER_QUEUE, queueName);
    ...
    async _set(options) {
        ...
        if (schema.valid) {
            ...
            const _path = path.join('/', PREFIX.PIPELINE_DRIVER_QUEUE, instance.queueName);
            result = await this._client.put(_path, instance);
        }
        return result;
    }
